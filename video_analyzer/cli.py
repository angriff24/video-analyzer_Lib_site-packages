...
def main():
    ...
    try:
        ...
        if args.start_stage <= 2:
            logger.info("Analyzing frames...S:\ProgramFiles\DMT\DMT_env\Lib\site-packages\video_analyzer\cli.py")
            ...
            if total_frames > 0:
                # 添加进度条初始状态
                sys.stderr.write("\r\033[KAnalyzing frames: 0/%d" % total_frames)
                sys.stderr.flush()
                ...
                for idx, frame in enumerate(frames):
                    ...
                    # 构建进度信息
                    progress_info = (
                        f"Analyzing frames: [{bar}] {percent}% "
                        f"({idx+1}/{total_frames} frames) - "
                        f"{frame_time:.1f}s/frame - "
                        f"ETA: {estimated_remaining_time/60:.1f}min"
                    )
                    
                    # 清除当前行并更新进度
                    sys.stderr.write(f"\r\033[K{progress_info}")
                    sys.stderr.flush()
                
                # 分析完成后添加换行
                sys.stderr.write("\n")
                sys.stderr.flush()
                logger.info(f"Frame analysis completed! Total time: {time.time()-start_time:.1f}s")
            else:
                logger.warning("No frames to analyze. Skipping frame analysis stage.")
