...
class VideoAnalyzer:
    ...
    def analyze_frame(self, frame: Frame) -> Dict[str, Any]:
        ...
        prompt = prompt.replace("[X]", f'[{frame.number}]')

        # prompt = 'describe the content of the image in detail, including objects, actions, and context.\n'
        ...
        try:
            ...
            logger.debug(f"\033[94m{prompt}\n\n{frame.path}\033[0m")
            logger.debug(f"\033[93mFrame {frame.number} analysis: {analysis_result.get('response', 'No response')}\033[0m")
